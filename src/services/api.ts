...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      console.log(
        "[API] 401 Unauthorized - clearing tokens and redirecting to login"
      );
  ...
  login: async (mobile: string, password: string) => {
    ...
    try {
      ...
      if (!response.data.access_token || !response.data.user) {
        throw new Error(
          "Invalid response structure: missing access_token or user"
        );
      }
...
export const tasksAPI = {
  ...
  createTask: async (data: {
    ...
    try {
      ...
      console.error(
        "[API] Failed to create single task:",
        error.response?.data || error.message
      );
      throw error;
    }
  ...
  createBulkTasks: async (data: {
    ...
    try {
      ...
      console.error(
        "[API] Failed to create bulk tasks:",
        error.response?.data || error.message
      );
      throw error;
    }
  ...
  updateTaskStatus: async (taskId: number, status: string) => {
    ...
    try {
      ...
      console.error(
        "[API] Failed to update task status:",
        error.response?.data || error.message
      );
      throw error;
    }
...
export const notificationsAPI = {
  getNotifications: async (params?: { skip?: number; limit?: number }) => {
    ...
    try {
      ...
      console.error(
        "[API] Failed to get notifications:",
        error.response?.data || error.message
      );
      throw new Error(handleApiError(error));
    }
  ...
  markAsRead: async (notificationId: number) => {
    ...
    try {
      ...
      console.error(
        "[API] Failed to mark notification as read:",
        error.response?.data || error.message
      );
      throw new Error(handleApiError(error));
    }
